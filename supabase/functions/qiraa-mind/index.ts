...
أخرج JSON فقط بالهيكل التالي:
{
  ...
  "strategic_intent": ["النية الاستراتيجية أو الهدف من السؤال"]
    ...
    const queryPromises = targetCountries.map(async (countryKey) => {
      ...
      let cQuery = adminSupabase.from("qiraa_companies").select("name, description, sector_main, country, valuation_min_usd, valuation_max_usd, total_funding_usd, growth_stage, launch_year, client_focus, income_streams, employees_range, revenue_estimate, growth_rate").order("total_funding_usd", { ascending: false }).limit(companiesPerCountry);
      ...
      let tQuery = adminSupabase.from("qiraa_transactions").select("company_name, sector_main, sectors_sub, round_type, round_amount_usd, growth_stage, valuation_min_usd, valuation_max_usd, total_funding_usd, investors, country, round_year, round_month, investor_types, transaction_type, transaction_type, valuation_currency").order("round_year", { ascending: false }).order("round_month", { ascending: false }).limit(transactionsPerCountry);
